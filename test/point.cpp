...
TEST_CASE("test point operators")
{
    ...
    { point<uint32_t> p(2, 3); CHECK((p += p3) == point<uint32_t>(6, 9)); }
    { point<uint32_t> p(2, 3); CHECK((p += 1u) == point<uint32_t>(3, 4)); }
    { point<uint32_t> p(4, 6); CHECK((p -= p2) == point<uint32_t>(2, 3)); }
    { point<uint32_t> p(4, 6); CHECK((p -= 1u) == point<uint32_t>(3, 5)); }
    { point<uint32_t> p(4, 6); CHECK((p *= p2) == point<uint32_t>(8, 18)); }
    { point<uint32_t> p(2, 3); CHECK((p *= 2u) == point<uint32_t>(4, 6)); }
    { point<uint32_t> p(4, 6); CHECK((p /= p2) == point<uint32_t>(2, 2)); }
    { point<uint32_t> p(4, 6); CHECK((p /= 2u) == point<uint32_t>(2, 3)); }
}
